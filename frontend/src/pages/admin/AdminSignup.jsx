...
export default function AdminSignup() {
  ...
  const onSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await signup(form.name, form.email, form.password, form.signupCode);
      toast.success('Account created — welcome to Almas Estates');
  ...
  return (
    <div className="min-h-screen bg-skyline-gradient flex items-center justify-center px-6 py-12">
      <form onSubmit={onSubmit} className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-sm">
        ...
        <h1 className="text-center text-slate text-sm mb-6 uppercase tracking-wide">Create Admin Account</h1>
        ...
        <div className="mb-6">
          <label className="block text-xs uppercase tracking-wide text-slate mb-1">Signup Code</label>
          <input required value={form.signupCode} onChange={(e) => update('signupCode', e.target.value)} className="w-full border border-black/10 rounded-lg px-3 py-2 focus-ring" />
          <p className="text-xs text-slate mt-1">Provided by your site administrator (ADMIN_SIGNUP_CODE).</p>
        </div>
        ...
        <p className="text-sm text-slate text-center mt-5">
          ...
          <Link to="/admin/login" className="text-teal hover:underline">Sign in</Link>
        </p>
      </form>
    </div>
  );
